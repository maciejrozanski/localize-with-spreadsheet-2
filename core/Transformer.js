...
var kotlinTemplateTransformer = {
    ...
    insert: function (input, newValues, options) {
        ...
        var generatedIndex = input.indexOf(kotlinTemplateTransformer.AUTOGENERATED_TAG);
        if (generatedIndex >= 0) {
            input = input.substr(0, generatedIndex);
        }

        var className = options && options.className ? options.className : 'T';
        var header = options && options.header ? options.header : 'package kotlin.example';

        var output = input + kotlinTemplateTransformer.AUTOGENERATED_TAG + EOL +
            header + EOL + EOL +
            'enum class ' + className + ' { ' + EOL + EOL +
            newValues + EOL +
            '}';
...
module.exports = {'ios': iOSTransformer, 'android': androidTransformer, 'json': jsonTransformer, 'dart': dartTransformer, 'kotlinTemplate': kotlinTemplateTransformer, 'dartTemplate': dartTemplateTransformer, '.net': dotNetTransformer };
